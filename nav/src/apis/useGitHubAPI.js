...
`
  ...
  const listBlogPosts = async () => {
    try {
      const { token, owner, repo } = getConfig()
      const url = `${GITHUB_API_BASE}/repos/${owner}/${repo}/contents/blog/src/content/posts`
  ...
  const getBlogPost = async (filename) => {
    try {
      const path = `blog/src/content/posts/${filename}`
  ...
  const createBlogPost = async (filename, content) => {
    try {
      const path = `blog/src/content/posts/${filename}`
      const message = `feat: 创建博客文章 ${filename}`

      // 创建新文件时不需要 sha
      return await updateFileContent(path, content, message, null)
    } catch (error) {
  ...
  // 更新博客文章
  const updateBlogPost = async (filename, content, sha) => {
    try {
      const path = `blog/src/content/posts/${filename}`
      const message = `chore: 更新博客文章 ${filename}`

      return await updateFileContent(path, content, message, sha)
    } catch (error) {
  ...
  // 删除博客文章
  const deleteBlogPost = async (filename, sha) => {
    try {
      const { token, owner, repo, branch } = getConfig()
      const path = `blog/src/content/posts/${filename}`
      const encodedPath = path.split('/').map(segment => encodeURIComponent(segment)).join('/')
      ...
      const message = `chore: 删除博客文章 ${filename}`
