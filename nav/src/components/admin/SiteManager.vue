          ...
          <div class="form-group">
            ...
            <div class="icon-input-group">
              <input
                ...
                ref="iconFileInput"
              >
              <button type="button" @click="triggerIconUpload" class="upload-btn">
                📁 选择图片
              </button>
            ...
            <div class="icon-preview" v-if="formData.icon">
              ...
              <button type="button" @click="clearIcon" class="clear-icon-btn">✕</button>
            </div>
          </div>
...
const handleIconUpload = async (event) => {
  ...
  try {
    ...
    // 生成文件名（使用站点域名或时间戳）
    let fileName = `icon-${Date.now()}.${file.name.split('.').pop()}`
    if (formData.value.url) {
      try {
        const url = new URL(formData.value.url)
        fileName = `${url.host}.${file.name.split('.').pop()}`
      } catch (e) {
        console.warn('无法解析URL，使用时间戳作为文件名')
      }
    }

    const localPath = `/sitelogo/${fileName}`

    // 创建预览URL（使用Base64，这样保存后也能显示）
    ...
    reader.onload = () => {
      ...
      // 缓存预览URL（使用Base64，永久有效）
      const oldPreview = iconPreviews.value.get(localPath)
      if (oldPreview && oldPreview.startsWith('blob:')) {
        URL.revokeObjectURL(oldPreview)
      }
      iconPreviews.value.set(localPath, base64Url)

      console.log(`✅ 图标预览已生成: ${fileName}`)
    }

    reader.readAsDataURL(blob)

    // 自动下载文件到本地（用户需要手动放到 public/sitelogo/ 目录）
    const downloadBlob = new Blob([arrayBuffer], { type: file.type })
    ...
    console.log(`📥 图标文件已下载: ${fileName}`)
    alert(`图标文件已下载: ${fileName}\n\n请将文件手动放到项目的 public/sitelogo/ 目录下`)

    // 缓存图标数据（用于后续可能的批量操作）
    const domain = fileName.replace(/\.[^.]+$/, '') // 移除扩展名
    pendingIcons.value.set(domain, {
      arrayBuffer,
      fileName,
      localPath,
    ...
    console.log(`✅ 图标文件已选择: ${fileName}`)
  } catch (error) {
    console.error('处理图标文件失败:', error)
    alert('处理图标文件失败，请重试')
  }
}
...
const handleSave = async () => {
  try {
    ...
    if (pendingIcons.value.size > 0) {
      console.log(`ℹ️ 有 ${pendingIcons.value.size} 个图标文件需要手动放到 public/sitelogo/ 目录`)
      const iconFiles = Array.from(pendingIcons.value.values()).map(icon => icon.fileName).join(', ')
      console.log(`📁 文件列表: ${iconFiles}`)
    }
