    ...
    <header class="modern-header glass-effect animate-slide-in-up">
      <div class="header-content">
        ...
        <div class="quick-actions animate-slide-in-right">
          ...
          <button @click="toggleTheme" class="action-btn glass-effect" title="切换主题">
            <span class="action-icon">{{ isDarkMode ? '☀️' : '🌙' }}</span>
            <span class="action-text">{{ isDarkMode ? '浅色' : '深色' }}</span>
          </button>
        </div>
            ...
            <div class="sites-grid">
              ...
              >
                ...
                <div class="site-actions">
                  <FavoriteButton 
                    :is-favorited="userStore.isFavorited(site.id)"
                    @toggle="addToFavorites(site)"
                    @click.stop
                  />
                </div>
              </div>
...
const handleImageError = (event) => {
  ...
  const colorIndex = siteName.charCodeAt(0) % colors.length
...
const addToFavorites = (site) => {
  const isCurrentlyFavorited = userStore.isFavorited(site.id)
  
  if (isCurrentlyFavorited) {
    // 取消收藏
    userStore.removeFavorite(site.id)
    showToast(`已取消收藏 ${site.name}`, 'info')
  } else {
    // 添加收藏
    ...
    if (success) {
      showToast(`已收藏 ${site.name}`, 'success')
    } else {
